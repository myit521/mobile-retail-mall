...
create index idx_category_status_sort on category (status, sort);
...
(
    biz_tag     varchar(64)                        not null comment '业务标识' primary key,
...
create index idx_memo_due_date on memo (due_date);
create index idx_memo_remind_time on memo (remind_time, is_reminded);
create index idx_memo_status on memo (status);
create index idx_memo_user_id on memo (user_id);
...
(
    ...
    constraint uk_out_trade_no_transaction_id unique (out_trade_no, transaction_id)
);
...
(
    ...
    create_user bigint        null comment '创建人',
    update_user bigint        null comment '修改人',
    constraint idx_phone_model_name unique (brand, model_name)
)
    comment '手机型号表' collate = utf8mb3_bin;

create index idx_phone_model_brand on phone_model (brand);
create index idx_phone_model_status_sort on phone_model (status, sort);
...
create index idx_product_status_update on product (status, update_time);
...
create index idx_position_code on product_location (position_code);
create index idx_shelf_code on product_location (shelf_code);
...
(
    ...
    constraint uk_product_phone_model unique (product_id, phone_model_id)
)
    comment '商品与手机型号关联表' collate = utf8mb3_bin;

create index idx_phone_model_id on product_phone_model (phone_model_id);
create index idx_ppm_phone_model on product_phone_model (phone_model_id);
create index idx_ppm_product on product_phone_model (product_id);
create index idx_product_id on product_phone_model (product_id);
...
create index idx_alert_product on stock_alert (product_id);
create index idx_alert_status on stock_alert (alert_status);
...
create index idx_check_record_plan on stock_check_record (plan_id);
create index idx_check_record_product on stock_check_record (product_id);
...
create index idx_stock_log_type_time on stock_log (change_type, create_time);
