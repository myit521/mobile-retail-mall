...
<template>
  <view class="history_order">
    <uni-nav-bar @clickLeft="goBack" left-icon="back" leftIcon="arrowleft" title="历史订单" statusBar="true" fixed="true"
      color="#ffffff" backgroundColor="#2F6BFF"></uni-nav-bar>
                  ...
                  ? record.orderDetailList.map((detail) => ({
                      ...
                      image:
                        ...
                        (typeof detail.image === 'string' &&
                          (!detail.image.trim() ||
                            detail.image.trim().indexOf('/pages/') === 0 ||
                            detail.image.trim().indexOf('pages/') === 0 ||
                            (detail.image.trim().indexOf('/') === 0 &&
                              detail.image.trim().indexOf('/static/') !== 0)))
                          ? '/static/imgDefault.png'
                          : detail.image,
                    }))
                  : [],
              }))
