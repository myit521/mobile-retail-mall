...
import {
  ...
  getTimeQuantumType,
  getTimeQuantumDishes as getTimeQuantumProducts,
...
export default class extends Vue {
  ...
  private getTimeWuantumData() {
    ...
    this.getReceivables()
    this.getTimeQuantumTypeData()
    this.getTimeQuantumProductData()
    this.getDaySalesVolumeData()
    this.getDiscount()
  }
  ...
  // 获取时间范围之内的菜品销售排行
  private getTimeQuantumProductData() {
    getTimeQuantumProducts({ start: this.stateTime, end: this.endTime })
      .then((res) => {
        if (res.data.code == 200) {
          const { data } = res.data
          let yData: number[] = []
