        ...
        >
          <template slot-scope="{ row }">
            ...
            <div class="before">
              ...
              <!-- TODO 商城模式：移除旧外卖流转按钮（当前为店内自提） -->
              <!-- <el-button
                v-if="row.status === 3"
                type="text"
                class="blueBug"
                @click="handleUnsupportedOrderAction(3, row.id)"
              >
                处理
              </el-button> -->
              <!-- TODO 商城模式：移除旧配送状态，保留当前订单状态即可 -->
              <!-- <el-button
                v-if="row.status === 4"
                type="text"
                class="blueBug"
                @click="handleUnsupportedOrderAction(4, row.id)"
              >
                完成
              </el-button> -->
            </div>
        ...
        <div class="order-middle">
          ...
          <div class="dish-info">
            ...
            <div class="dish-all-amount">
              <label>商品小计</label>
              <span>￥{{ diaForm.amount ? diaForm.amount.toFixed(2) : '0.00' }}</span>
            </div>
          </div>
        </div>
          ...
          <div class="amount-info">
            ...
            <div class="amount-list">
              <div class="dish-amount">
                ...
                <span class="amount-price">￥{{ diaForm.amount ? ((diaForm.amount.toFixed(2) * 100) / 100) : '0.00' }}</span>
              </div>
              <!-- TODO 商城模式：移除旧外卖费用项 -->
              <!-- <div class="send-amount">
                <span class="amount-name">旧费用项：</span>
...
export default class extends Vue {
  ...
  private cancelDialogVisible = false //订单处理弹窗
  ...
  private cancelOrderReasonList = [
    { value: 1, label: '当前订单较多，暂时无法处理' },
    { value: 2, label: '商品库存不足，暂时无法处理' },
    { value: 3, label: '门店当前暂停营业' },
    { value: 0, label: '自定义原因' },
  ]
