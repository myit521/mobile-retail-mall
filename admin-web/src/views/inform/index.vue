      ...
      <div v-if="baseData.length > 0" class="informList">
        <div v-for="(item, index) in baseData" :key="index">
          ...
          <div v-if="item.type === 2" class="item">
            <div class="tit">
              <i>急</i><span>【待确认】</span>{{ item.arrNew[0]
          ...
          <div v-if="item.type === 3" class="item">
            <div class="tit">
              <span>【待处理】</span>{{ item.arrNew[0]
              }}<span class="fontOrderTip" @click="handleSetStatus(item.id)"><router-link :to="'/order?status=' + 3">{{
                item.arrNew[1]
              }}</router-link></span>{{ item.arrNew[2]
              }}<span class="time">{{ item.createTime }}</span>
            </div>
          ...
          >
            <div :class="isActive ? 'titAlready' : ''">
              ...
              <div v-if="shopShow && showIndex === index" class="orderInfo">
                <p>
                  <span><label>下单时间：</label>{{ item.details.orderTime }}</span>
                </p>
                <p>
                  下单用户：{{ item.details.userName || '-' }}，门店信息：{{ item.details.address }}
                </p>
                <p>
                  <span><label>商品明细：</label>{{ item.details.orderDishes }}</span>
                </p>
              </div>
            </div>
          </div>
