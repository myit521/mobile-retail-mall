              ...
              <template slot-scope="{ row }">
                ...
                <div class="before">
                  ...
                  <el-button
                    ...
                    type="text"
                    class="blueBug"
                    @click="handleUnsupportedOrderAction(3, row.id, $event)"
                  >
                    处理
                  </el-button>
                </div>
    ...
    >
            ...
            <div class="amount-list">
              <div class="dish-amount">
                ...
                <span class="amount-price">￥{{
                  diaForm.amount ? (diaForm.amount.toFixed(2) * 100) / 100 : '0.00'
                }}</span>
              </div>
      ...
      <span v-if="dialogOrderStatus !== 6" slot="footer" class="dialog-footer">
        ...
        <!-- <el-button
          ...
          type="primary"
          @click="handleUnsupportedOrderAction(4, row.id, $event)"
          >完 成</el-button
        > -->
        <el-button
...
export default class extends Vue {
  ...
  // 完成订单（dashboard 仅保留确认后的查看提示）
  handleUnsupportedOrderAction(status: number, id: string, event) {
    event.stopPropagation()
    // 当前商城模式不走配送/完成接口，直接提示不支持
    this.$message.warning('请前往订单管理页处理该操作')
  }
  // 查看详情
