  ...
  <div class="navbar">
    ...
    >
      <el-radio-group v-model="setStatus">
        <el-radio :label="1">
          营业中
          <span>当前系统处于正常营业状态，可继续处理新订单；如需临时暂停，可切换为暂停中。</span>
        </el-radio>
        <el-radio :label="0">
          暂停中
          <span>当前系统处于暂停营业状态，前台将不再进入新的待处理订单，可随时切回营业中恢复。</span>
        </el-radio>
      </el-radio-group>
      <span slot="footer"
            class="dialog-footer"
...
export default class extends Vue {
  ...
  get getStoreId() {
    let storeId = ''
    if ((UserModule as any).storeId) {
      storeId = (UserModule as any).storeId
  ...
  webSocket() {
    ...
    // 带 Token 鉴权参数
    let socketUrl = process.env.VUE_APP_SOCKET_URL + clientId + '?token=' + UserModule.token
    console.log('WebSocket 连接地址:', socketUrl)
    ...
    this.websocket.onopen = function () {
      ...
      that.reconnectCount = 0 // 重置重连次数
      that.startHeartbeat() // 启动心跳
    }
    ...
    this.websocket.onmessage = function (msg) {
      ...
      console.log('收到 WebSocket 消息:', jsonMsg)
      
      if (jsonMsg.type === 'pong') {
        // 心跳响应，不处理
        return
      }
      ...
      that.$notify({
        title: jsonMsg.type === 1 ? '新订单提醒' : '订单提醒',
        duration: 0,
        ...
        message: `${
          jsonMsg.type === 1
            ? `<span>您有 1 个<span style=color:#419EFF>新订单待处理</span>，${jsonMsg.content}，请及时处理</span>`
  ...
  attemptReconnect() {
    ...
    if (this.reconnectCount >= this.maxReconnectCount) {
      ...
      that.$notify({
        title: '提示',
        message: 'WebSocket 连接断开，请刷新页面重试',
        type: 'warning',
        duration: 5000,
      })
      return
    ...
    this.reconnectCount++
    console.log(`准备重连，第 ${this.reconnectCount} 次`)
